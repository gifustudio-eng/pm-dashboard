...
// Pastikan Ad Account ID selalu berformat act_XXXXXXXXX
...
async function insertClient(client) {
  const { data, error } = await supabase
    ...
    .insert({
      ...
      meta_ad_account_id: normalizeAdAccountId(client.metaId || client.metaAdAccountId),
...
async function updateClient(id, updates) {
  const patch = { ...updates };
  if (patch.meta_ad_account_id) patch.meta_ad_account_id = normalizeAdAccountId(patch.meta_ad_account_id);
  if (patch.metaId) { patch.meta_ad_account_id = normalizeAdAccountId(patch.metaId); delete patch.metaId; }
...
module.exports = {
  ...
  // clients
  getAllClients, getClientById, insertClient, updateClient, deleteClient,
  // insights
